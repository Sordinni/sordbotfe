...
async function handleAdminResponse(sock, msg) {
  ...
  if (response === 'autorizar') {
    ...
    try {
      ...
      console.log(`${traceId} ⚠️ Falha ao obter PN via lidMapping:`, err.message);
    }

    if (!userJid) {
      userJid = userLid.replace('@lid', '@s.whatsapp.net');
    ...
    console.log(`${traceId} ✅ Mensagem enviada para ${userJid}`);
  } 
...
• 🎨 Envie código do emoji.gg → sticker.
...
• \`renomear "nome" "autor"\` → renomeia sticker
